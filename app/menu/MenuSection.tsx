...
function DishRow({ dish, last, sectionId, photo }: { dish: { name: string; composition: string; kcal: number; price: number }; last: boolean; sectionId: string; photo: string }) {
  ...
  const cartItem = items.find(i => i.dishId === itemId || i.id === itemId)
        ...
        {isAuthenticated && (
          <button
            ...
            style={{
              ...
              transform: isFavorite ? "scale(1.12)" : "scale(1)",
              flexShrink: 0,
            }}
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill={isFavorite ? "#D62828" : "none"} stroke={isFavorite ? "#D62828" : "#888"} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" />
            </svg>
          </button>
        )}
        {qty === 0 ? (
          <button className="product-button" onClick={handleAdd}>В корзину</button>
        ) : (
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <button onClick={() => updateQty(itemId, qty - 1)} style={{ width: "34px", height: "34px", borderRadius: "50%", border: "2px solid #FF4D00", background: "none", color: "#FF4D00", fontSize: "20px", cursor: "pointer", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>-</button>
