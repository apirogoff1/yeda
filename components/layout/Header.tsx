...
const navItems = [
  { label: 'Главная',    color: '#FF5A1F', y: 10  },
  { label: 'Меню',       color: '#42B883', y: -14 },
  { label: 'Как работает', color: '#25A9E0', y: 6  },
  { label: 'Подписка',   color: '#B85CFF', y: -10 },
  { label: 'Доставка',   color: '#FF5A1F', y: 14  },
  { label: 'О компании', color: '#42B883', y: -6  },
  { label: 'FAQ',        color: '#25A9E0', y: 8   },
  { label: 'Контакты',   color: '#B85CFF', y: -12 },
]
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          {authed ? (
            ...
            <button style={{
              fontFamily: 'var(--font-geologica)', fontWeight: 900, fontSize: '18px',
              background: 'none', border: 'none', cursor: 'pointer', color: '#433932', padding: '8px 16px',
            }}>
              Войти
            </button>
          )}
          <button
            onMouseEnter={e => {
              (e.target as HTMLButtonElement).style.transform = 'translateY(-2px) scale(1.03)'
