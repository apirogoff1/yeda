    ...
    <>
      <style>{`
        @media (max-width: 768px) {
          ...
          .header-root { padding: 0 16px !important; height: 72px !important; }
      ...
      <header className="header-root" style={{
        ...
        <div className="header-inner" style={{
          ...
          <div className="header-desktop-right" style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            {userRole ? (
              <>
                <Link href="/dashboard"
      ...
      {menuOpen && (
        <div className="mobile-menu-overlay" style={{
          ...
          zIndex: 49,
          background: 'linear-gradient(160deg, #FFE8C0 0%, #FFE4D8 50%, #FFE4E8 100%)',
            ...
            {navItems.map((item) => (
              <Link
                ...
                style={{
                  ...
                  boxShadow: '0 2px 12px rgba(255,100,0,0.08)',
            ...
            {userRole ? (
              <>
                <Link href="/dashboard" onClick={() => setMenuOpen(false)} style={{
                  fontFamily: 'var(--font-geologica)', fontWeight: 900, fontSize: '20px',
                  color: '#1A6FD4', textDecoration: 'none', padding: '12px 0',
                }}>Личный кабинет</Link>
                <button onClick={() => { handleLogout(); setMenuOpen(false) }} style={{
                  fontFamily: 'var(--font-geologica)', fontWeight: 900, fontSize: '20px',
                  color: '#433932', background: 'none', border: 'none', cursor: 'pointer',
                }}>Выйти</button>
              </>
            ) : (
              <Link href="/login" onClick={() => setMenuOpen(false)} style={{
                fontFamily: 'var(--font-geologica)', fontWeight: 900, fontSize: '20px',
                color: '#433932', textDecoration: 'none', padding: '12px 0',
              }}>Войти</Link>
            )}
