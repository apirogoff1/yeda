  ...
  return (
    <div>
      {lightbox && (
        ...
        >
          <div
            className="relative w-2/3 max-h-[80vh] rounded-2xl overflow-hidden shadow-2xl"
      ...
      {loadingDoctors ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {doctors?.map((doctor: {
            ...
            >
              <div
                className="w-full h-64 rounded-2xl mb-4 overflow-hidden bg-blue-100 flex items-center justify-center cursor-zoom-in ring-1 ring-gray-200 hover:ring-2 hover:ring-blue-300 transition-all duration-300"
                onClick={(e) => {
                  if (doctor.photoUrl) {
                    e.preventDefault()
                    e.stopPropagation()
